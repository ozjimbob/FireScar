...
## Slightly modified version to handle split regimes
launch_sim_split=function(idx){
  print(idx)
  dim(sim_def_frame)
  this_sim=sim_def_frame[idx,]
  
  output=scar_sim(this_sim$SIM_ID,this_sim$FIELD_SIZE,this_sim$SIM_LENGTH,c(this_sim$FIRE_FREQ_1,this_sim$FIRE_FREQ_2),c(this_sim$FIRE_PROB_1,this_sim$FIRE_PROB_2),this_sim$MORTALITY_ON,this_sim$F_MORTALITY_ON,
                  this_sim$MORT_b1,this_sim$MORT_b2,this_sim$MORT_p1,this_sim$MORT_p2,this_sim$MORT_p3,
                  this_sim$MORT_F_b1, this_sim$MORT_F_b2,this_sim$MORT_F_b3, this_sim$MORT_F_p1,this_sim$MORT_F_p2,this_sim$MORT_F_p3, this_sim$MORT_F_p4,FIRE_PATTERN=this_sim$SPLIT_YEAR)
...
launch_sim=function(idx){
  ...
  output=scar_sim(this_sim$SIM_ID,this_sim$FIELD_SIZE,this_sim$SIM_LENGTH,this_sim$FIRE_FREQ,this_sim$FIRE_PROB,this_sim$MORTALITY_ON,this_sim$F_MORTALITY_ON,
                  this_sim$MORT_b1,this_sim$MORT_b2,this_sim$MORT_p1,this_sim$MORT_p2,this_sim$MORT_p3,
                  this_sim$MORT_F_b1, this_sim$MORT_F_b2,this_sim$MORT_F_b3, this_sim$MORT_F_p1,this_sim$MORT_F_p2,this_sim$MORT_F_p3, this_sim$MORT_F_p4,FIRE_PATTERN=FALSE)
  write.csv(output$tree_frame,paste0(this_sim$output_dir,"/tree_frame_",idx,".csv"))
  this_fire=output$fire_record
  save(this_fire,file=paste0(this_sim$output_dir,"/fire_record_",idx,".Rdata"))
  this_veg_matrix=output$veg_matrix
...
scar_sim=function(SIM_ID,FIELD_SIZE,SIM_LENGTH,FIRE_FREQ,FIRE_PROB,MORTALITY_ON,F_MORTALITY_ON,
                  ...
                  MORT_F_b1,MORT_F_b2,MORT_F_b3,MORT_F_p1,MORT_F_p2,MORT_F_p3,MORT_F_p4,FIRE_PATTERN=FALSE){
  ...
  burn_veg=function(){
    ...
    prob_burn_field=matrix(runif(COUNT_TREES,0,1)<(this_FIRE_PROB/100),nrow=FIELD_SIZE,ncol=FIELD_SIZE) #Fire pixels as logic to allow use as cell selector
  ...
  for(idx in 1:SIM_LENGTH){
    ...
    if(!FIRE_PATTERN){
      this_FIRE_FREQ=FIRE_FREQ
      this_FIRE_PROB=FIRE_PROB
    } else {
      if(idx < FIRE_PATTERN){
        this_FIRE_FREQ=FIRE_FREQ[1]
        this_FIRE_PROB=FIRE_PROB[1]     
      }else{
        this_FIRE_FREQ=FIRE_FREQ[2]
        this_FIRE_PROB=FIRE_PROB[2]
      }
    }
    ...
    fire_year = runif(1,0,1) < this_FIRE_FREQ/100
  ...
  tree_frame$FIRE_FREQ=paste0(FIRE_FREQ,collapse="-")
  tree_frame$FIRE_PROB=paste0(FIRE_PROB,collapse="-")                      
...
plot_dendro=function(tree_frame){
  ...
  for(idx in 1:number_of_trees){
    this_tree=subset(tree_frame,tree_frame$ID==idx)
